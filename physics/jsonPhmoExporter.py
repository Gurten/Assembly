# Author: Gurten
# Find the version for Blender 2.63 at https://github.com/Gurten/HaloOnlineTagTool/blob/master/phmo_coll/jsonPhmoExporter.py
...
def serialisePolyhedron(obj):
    ...
    for v in local_verts:
        key = keyForCoords((c for c in v), dp_epsilon)
        verts[key] = v
    planes = {}
    ...
    if len(extreme_pts) < 4:
        print("Object: \'%s\' did not have enough extreme points to create a 3D hull." % (obj.name))
        return None
...
class ExportJson(bpy.types.Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        output = []
        for obj in objs:
            serializedPoly = serialisePolyhedron(obj)
            if serializedPoly:
                output.append(serializedPoly)
